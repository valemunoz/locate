		 ...
		 $(function() {
    		$( "#hasta" ).datepicker({dateFormat:"yy-mm-dd"});
  			...
  			function loadLista()
  			{
  				...
  				var estado=document.getElementById("opc_estado").value;
  				$("#resultado").html("<div><img src='img/load.GIF'></div>");
  				$("#resultado").load("query.php", 
					{tipo:4,id:id_usuario,desde:desde,hasta:hasta, est:estado} 
				,function(){	
				}
				);
  			}
...
<div id="buscador">
<fieldset>
<legend>Filtro Log Accesos</legend>
...
<table id=table_filtro>
		...
		<tr><td><input class="input_filtro" autocomplete=off type="text" name="id" id="id" maxlength="12"/> </td>
		<td><select id="opc_estado">
			<option value=10>Todos</option>
							<option value=0>Exitoso</option>
							<option value=1>Error Clave</option>
							<option value=2>Fracaso</option>
						</select> 
		</td>
