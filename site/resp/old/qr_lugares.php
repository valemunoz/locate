...
//$CM_path_base="http://www.check.geodoomla.com/app/site"; 
//$CM_path_base2="http://check.geodoomla.com/app/site"; 
$CM_path_base="http://localhost/doomla/demos/check";

if(substr(strtolower($data_server[0]),0,strlen($CM_path_base))==$CM_path_base or substr(strtolower($data_server[0]),0,strlen($CM_path_base2))==$CM_path_base2)
{
...
if($_REQUEST['tipo']==1 and $est_sesion==0) //lista de empresas
	...
	}elseif($_REQUEST['tipo']==4 and $est_sesion==0)//busca GEO
	{
		...
		{
			$direc=buscarDireccionOSM($direccion_completa." chile");
			//print_r($direc);
			...
			}else //GOOOGLE
			{
				...
				$direc=getDireccionGoogle($direccion_completa." Chile");
				//print_r($direc);
