			...
			<?PHP
				...
				foreach($data as $i => $dat)
				{
					...
					<tr>
						...
						<TD><input type="button" onclick="detalleRegistro(<?=$dat[0]?>);" value="Detalle"><input type="button" id="opener" value="Ver Mapa" onclick="limpiarMapa();addMarcador('img/place.png','30,30',<?=$dat_empresa[5]?>,<?=$dat_empresa[6]?>,'','<?=ucwords($dat_empresa[0])?><br> <?=ucwords($dat_empresa[1])?> #<?=$dat_empresa[2]?><br><?=ucwords($dat_empresa[4])?>');verMarcadores();OpenModal();"></TD>
					</tr>
					<?php
				}
				?>
...
}elseif($_REQUEST['tipo']==2)
{
	...
	if(count($usuario)>0)
	{
		...
		if($_REQUEST['clave']==$usuario[6] and $usuario[8]==1 and $cliente[1]==0)
		{
			inicioSesion_web($_REQUEST['mail'],$usuario[1],$usuario[0],$usuario[3],$usuario[10],$cliente[6]);		
			?>
		<script>
...
}elseif($_REQUEST['tipo']==6 and $est_sesion==0) //mapa
{
	...
	foreach($usuarios as $us)
	{
		...
		if(count($registro)>0)
		{
			...
			?>
			<script>
				addMarcador('img/place.png','30,30','<?=$lat?>','<?=$lon?>','','<?=$html?>');				
			</script>
			<?php
			}
			$i++;
...
?>
	<script>
		...
		<!--	<tr class="titulo">-->
			...
			<tr>			
				<td>Fecha agenda</td>
				<td><input type="text" class="input_filtro" autocomplete=off id="fec_agenda" name="fec_agenda" maxlength="50" class="txtFecha"/></td>
			</tr>
			<tr>			
				<td>Comuna</td>
				<td>
			...
			<tr>
				<td></td><td><input type="button" value="Agendar" onclick="validaAgenda();"></td>
			</tr>
		</table>
...
?>
	<script>
		...
		<!--	<tr class="titulo">-->
			...
			<tr>			
				<td>Fecha agenda</td>
				<td><input type="text" class="input_filtro" autocomplete=off id="fec_agenda" name="fec_agenda" maxlength="50" value="<?=$agenda[0][3]?>" class="txtFecha"/></td>
			</tr>
			<tr>			
				<td>Visita</td>
				<td>
...
}elseif($_REQUEST['tipo']==12 and $est_sesion==0)
{
	...
	upAgenda("estado=".$_REQUEST['estado'].", fecha_agenda='".$_REQUEST['fec']."', id_empresa=".$_REQUEST['visita'].", id_categoria_usuario=".$_REQUEST['motivo'].", descripcion='".$_REQUEST['det']."'",$_REQUEST['agenda']);
	?>
	<script>
		CloseModal();
		</script>
...
?>
		...
		<!--	<tr class="titulo">-->
			...
			foreach($data as $dat)
			{
				...
				<Td style="width:15%;"><?=$dat[3]?></Td>
				<td style="width:20%;"><?=$est?></td>
				<Td style="width:25%;"><a href="<?=$onclick?>"><img src="img/<?=$img?>" title=<?=$txt_img?>></a></Td>
...
?>
		...
		<!--	<tr class="titulo">-->
			...
			<tr>
				<td></td><td><input type="button" value="Guardar" onclick="saveItem();"></td>
			</tr>
		</table>
		<div id='msg_agenda' class="msg_error"></div>
<?php		
}elseif($_REQUEST['tipo']==17 and $est_sesion==0)
{
	$nombre=$_REQUEST['nom'];
	$id_cliente=$_SESSION['id_cliente_web'];
	$data=array();
