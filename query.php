...
}elseif($_REQUEST['tipo']==5) //load registros por usuario
{
	...
	<?php
		...
		}elseif(trim($reg[8])!="" and ($_SESSION['app']==1 or $_SESSION['app']==2 or ($_SESSION['app']==3 or $_SESSION['app']==4 or $_SESSION['app']==5 or $_SESSION['app']=6)))
		{
			$fec_termino=trim($reg[8]);
			//$fecha_actual2 = strtotime ( '-3 hours ' , strtotime ( $fec_termino ) ) ;
		...
		}elseif(trim($reg[8])=="" and ($_SESSION['app']==5)) //usuario + check-in+agenda + stock
		{
			...
			<?php
		}elseif(trim($reg[8])=="" and ($_SESSION['app']==6)) //usuario + check-in+agenda + Pedido
		{
...
}elseif($_REQUEST['tipo']==10 and $est_sesion==0)
{
	...
	<?php
	}elseif($_SESSION['app']==6)
	{
		$prod_detalle=getProductosRegistro($_REQUEST['registro']);
		$html2="<br><h3>Pedido</h3>";
		$html2 .="<table><tr align=center class=txt_header><td width=20%>Producto</td><td width=20%>Valor</td><td width=20% >Cantidad</td></tr>";
		foreach($prod_detalle as $prod_d)
		{
			$producto=getProducto($prod_d[1]);
			$html2 .="<tr class=cont_pedido ><td align=left width=30%><h5>".ucwords($producto[2])."</h5></td><td align=center width=20%><h5>".$prod_d[11]."</h5></td><td align=center width=20%><h5>".$prod_d[12]."</h5></td></tr>";
		}
		$html2 .="</table>";
...
}elseif($_REQUEST['tipo']==11) //load agendas por usuario
{
	...
	<div class="ui-bar ui-bar-a" style="text-align:left;">
		<img src="images/map.png" onclick="ordenarAgenda();" width=24px height=24px  id="icon_li"> <img id="icon_li2" src="images/refresh.png" onclick="loadAgendas();"><img id="icon_li3" src="images/clock.png" onclick="ordenarAgendaReloj();">
	</div>
	<?php
	...
	?>	
				<li class="<?=$color?>"><a class="<?=$color?>" href="javascript:verDetalleAgenda('<?=ucwords($datos_empresa[0])?>','<?=ucwords($datos_empresa[0])?>','<?=ucwords($ag[7])?>','<?=$direccion?>','<?=ucwords($datos_empresa[7])?>','<?=ucwords($ag[8])?>');"><?=ucwords(strtolower($datos_empresa[0]))?> <span class=txt_mini> <?=$distancia?><span></a>
...
}elseif($_REQUEST['tipo']==13) //load agendas por reloj
{
		$agenda=getAgendas(" and id_usuario=".$_SESSION["id_usuario"]." and fecha_agenda = '".getFechaDia()."' and estado!=1 order by hora asc");
	?>
	<div class="ui-bar ui-bar-a" style="text-align:left;">
		<img src="images/map.png" onclick="ordenarAgenda();" width=24px height=24px  id="icon_li"> <img id="icon_li2" src="images/refresh.png" onclick="loadAgendas();"><img id="icon_li3" src="images/clock.png" onclick="ordenarAgendaReloj();">
	</div>
	...
	?>	
				<li class="<?=$color?>"><a class="<?=$color?>" href="javascript:verDetalleAgenda('<?=ucwords($datos_empresa[0])?>','<?=ucwords($datos_empresa[0])?>','<?=ucwords($ag[7])?>','<?=$direccion?>','<?=ucwords($datos_empresa[7])?>','<?=ucwords($ag[8])?>');"><?=ucwords(strtolower($datos_empresa[0]))?> <span class=txt_mini> Hora: <?=$ag[8]?><span></a>
					<a href="javascript:verMapa(<?=$datos_empresa[5]?>,<?=$datos_empresa[6]?>,'<?=$texto_html?>');" data-rel="popup" data-icon="search" data-position-to="window" data-transition="pop">Mapa</a>
				</li>
	<?php
	}
	...
	<?php
}elseif($_REQUEST['tipo']==14 and $est_sesion==0) //checkout 5 + pedido
{
	updateRegistro($_REQUEST['registro'],"fecha_check_out='".getFecha()."', detalle='".$_REQUEST['detalle']."'");	
	//add detalle producto
  $producto=explode("|",$_REQUEST['prod_id']);
  $producto_ini=explode("|",$_REQUEST['prod_cant']);
  $cliente=getCliente($_SESSION['id_cliente']);
  $msg="<br>Pedido Cliente ".$_REQUEST['empresa']."<br><br>";
  foreach($producto as $i => $pro)
  {
  	$data=array();
  	$product=getProductosQR(" and id_producto=".$pro."");
  	$data[]=$pro;
  	$data[]=$_SESSION["id_usuario"];
  	$data[]=getFecha();
  	$data[]=$_SESSION["id_cliente"];
  	...
  	$data[]=$_REQUEST['detalle'];
  	$data[]=$_REQUEST['registro'];	
  	  	if(!is_numeric($product[0][5]))
  	  $product[0][5]=0;
  	$data[]=$product[0][5]; //valor  	
  	$data[]=$producto_ini[$i]; //cantidad
  	$msg .="<br> ".$product[0][2]." - ".$product[0][5]." - ".$producto_ini[$i]."";
  	addProductoDetPedido($data);	
  	
  }
  if(trim($cliente[9])!="")
  	{
  		$msg .="<br><br> Detalle:".$_REQUEST['detalle']."";
  		sendMail($cliente[9],$msg,"Pedido Usuario ".$_SESSION['nombre']."");
  	}
